...
用户**不用记触发词**。禁止要求改口，禁止只认「记住」「忘掉」。

看**意图**，原话只是例子：

| 意图 | 怎么判断 | 原话举例（不是清单） |
| --- | --- | --- |
| 写入 | 要以后都按这个做 | 记住、记下来、以后都这样、写进规范、当成默认、保存这条 |
| 作废 | 某条已记下的偏好不要了 | 忘掉、删除、取消、作废、去掉、那条不要了、删掉刚才记的 |
| 覆盖 | 同一对象改成新规则 | 改成用 text、橙色那条换成品牌橙 |
| 同意写入 | 对刚才复述表示同意 | 对、好、行、可以、要、嗯、是、OK、记吧 |
| 拒绝写入 | 只要这一次或先别记 | 这次而已、先别、不要记、改范围 |

「删除这个按钮 / 删掉这个文件 / 去掉这个接口」是改代码，**不是**作废记忆。  
「删除橙色那条 / 刚才记的删掉 / 这条不要了」才是作废记忆。

分不清时问一句：是作废项目记忆，还是只改代码？不要连续追问。

指代「刚才那条」：本轮刚写入的 id；对不上或有多条，先列出候选再作废。
...
同意 → `add`。拒绝或只要这一次 → 不写。改范围 → 改完再问。未表态 → 本轮不写。

默认范围是最窄：当前模块或组件类型。只有用户说全部、以后、全局、所有页面才放宽到本面全局。

## 作废

命中作废意图：`forget <id>` 或 `delete <id>`（同义）。作废条进 `archive.md`，INDEX 立即不再出现。读路径看不到的才算作废。

覆盖旧条：先 forget 旧 id，再 add 新条，新条「取代」写旧 id。禁止两条对着干的 active 规则并存。用户改口时直接覆盖，不必先说「忘掉」。
...
| --- | --- |
| 「记住操作列不要橙色」 | 复述后 `add` |
| 「以后都按这个做」「记下来」 | 同上，不要求说「记住」 |
| 「金额一律两位小数」PHP 已有 Money | 不重复记；admin 展示若缺口只记 admin |
| 「这个弹窗先改抽屉」 | 不记 |
| 「营销 CTA 可用品牌橙」vs 旧的禁止橙 | 覆盖：forget 旧条再 add |
| 「圆角就 8px」 | 改写成 token；改不成则拒绝记 |
| 「不要那种颜色」 | 不记，先问范围 |
| 「这个危险按钮用红」 | 不升格为全部按钮 |
| 「以后注释都按这段」 | 可记，存模板到示例 |
| 「橙色那条取消 / 删除 / 不要了」 | forget |
| 「删除这个按钮」 | 改代码，不动记忆 |
| 「删掉刚才记的」 | forget 本轮那条；多条则先列出来 |
| 「这里不要 Tabs」 | 默认只记当前模块 |
| 「回复短一点」 | 不记 |
| 找不到仓库根 | 不记，说明原因 |
